...
static void ReportBadArrayIndex(const char* pWhere, ucell ix, size_t arraySize)
{
	char buff[64];
#if defined(WIN32)
...
namespace OArray
{
    ...
    ClassVocabulary* gpArrayClassVocab = nullptr;

    oArrayStruct* createArrayObject(ClassVocabulary *pClassVocab)
    {

        ALLOCATE_OBJECT(oArrayStruct, pArray, pClassVocab);
    ...
    FORTHOP(oArrayResizeMethod)
	{
		...
		if (a.size() != newSize)
		{
			if (a.size() > newSize)
			{
				...
				for (ucell i = newSize; i < a.size(); i++)
				{
			...
			else
			{
				...
				a.resize(newSize);
				for (ucell i = a.size(); i < newSize; i++)
				{
					a[i] = nullptr;
				}
			}
    ...
    FORTHOP(oArrayFromMemoryMethod)
    {
        ...
        for (ucell i = offset; i < oldSize; i++)
        {
	...
	void objectArrayQuicksortStep(ForthCoreState* pCore, ForthObject* pData, cell left, cell right)
	{
		if (left < right)
		{
			cell pivot = objectArrayQuicksortPartition(pCore, pData, left, right);
    ...
    FORTHOP(oArrayIterInsertMethod)
	{
		...
		if (oldSize >= ix)
		{
			...
			POP_OBJECT(newObj);
			// add dummy object to end of array
			a.resize(oldSize + (ucell)1);
			if ((oldSize > 0) && (ix < oldSize))
			{
				// move old entries up by size of ForthObject
				memmove(&(a[ix + (ucell)1]), &(a[ix]), sizeof(ForthObject) * (oldSize - ix));
			}
			a[ix] = newObj;
			SAFE_KEEP(newObj);
		}
    ...
    ClassVocabulary* gpBagClassVocab = nullptr;

    oBagStruct* createBagObject(ForthObject& destObj, ClassVocabulary *pClassVocab)
    {

        ALLOCATE_OBJECT(oBagStruct, pBag, pClassVocab);
    ...
    FORTHOP(oBagFromMemoryMethod)
    {
        ...
        oBag& a = *(pBag->elements);
        ucell offset = SPOP;
        ucell numElements = SPOP;
        bagElement* pSrc = (bagElement*)(SPOP);
        ucell newSize = numElements + offset;
        size_t oldSize = a.size();

        // NOTE: overlapping source and dest will result in undefined behavior

        // increase refcount on all source objects
        for (ucell i = 0; i < numElements; i++)
        {
	...
	{
		ClassVocabulary *pClassVocab = (ClassVocabulary *)(SPOP);
		ALLOCATE_OBJECT(oByteArrayStruct, pArray, pClassVocab);
        pArray->pMethods = pClassVocab->GetMethods();
		pArray->refCount = 0;
	...
	{
		ClassVocabulary *pClassVocab = (ClassVocabulary *)(SPOP);
		ALLOCATE_OBJECT(oShortArrayStruct, pArray, pClassVocab);
        pArray->pMethods = pClassVocab->GetMethods();
        pArray->refCount = 0;
	...
	{
		ClassVocabulary *pClassVocab = (ClassVocabulary *)(SPOP);
		ALLOCATE_OBJECT(oIntArrayStruct, pArray, pClassVocab);
        pArray->pMethods = pClassVocab->GetMethods();
        pArray->refCount = 0;
	...
	{
		ClassVocabulary *pClassVocab = (ClassVocabulary *)(SPOP);
		ALLOCATE_OBJECT(oLongArrayStruct, pArray, pClassVocab);
        pArray->pMethods = pClassVocab->GetMethods();
        pArray->refCount = 0;
	...
	{
		ClassVocabulary *pClassVocab = (ClassVocabulary *)(SPOP);
		ALLOCATE_OBJECT(oDoubleArrayStruct, pArray, pClassVocab);
        pArray->pMethods = pClassVocab->GetMethods();
        pArray->refCount = 0;
    ...
    {
        ClassVocabulary *pClassVocab = (ClassVocabulary *)(SPOP);
        ALLOCATE_OBJECT(oStructArrayStruct, pArray, pClassVocab);
        pArray->pMethods = pClassVocab->GetMethods();
        pArray->refCount = 0;
    ...
    FORTHOP(oStructArraySetTypeMethod)
    {
        GET_THIS(oStructArrayStruct, pArray);
        oStructArray& a = *(pArray->elements);
        pArray->pVocab = (StructVocabulary *)SPOP;
        if (pArray->pVocab != nullptr)
        {
	...
	{
		ClassVocabulary *pClassVocab = (ClassVocabulary *)(SPOP);
		ALLOCATE_OBJECT(oPairStruct, pPair, pClassVocab);
	...
	{
		ClassVocabulary *pClassVocab = (ClassVocabulary *)(SPOP);
		ALLOCATE_OBJECT(oTripleStruct, pTriple, pClassVocab);
    ...
    void AddClasses(OuterInterpreter* pOuter)
	{
        ...
        pOuter->AddBuiltinClass("ArrayIter", kBCIArrayIter, kBCIIter, oArrayIterMembers);

        ClassVocabulary* pVocab = pOuter->AddBuiltinClass("Bag", kBCIBag, kBCIIterable, oBagMembers);
        pVocab->SetCustomObjectReader(customBagReader);
        pOuter->AddBuiltinClass("BagIter", kBCIBagIter, kBCIIter, oBagIterMembers);

        pVocab = pOuter->AddBuiltinClass("ByteArray", kBCIByteArray, kBCIIterable, oByteArrayMembers);
        pVocab->SetCustomObjectReader(customByteArrayReader);
        pOuter->AddBuiltinClass("ByteArrayIter", kBCIByteArrayIter, kBCIIter, oByteArrayIterMembers);

        pVocab = pOuter->AddBuiltinClass("ShortArray", kBCIShortArray, kBCIIterable, oShortArrayMembers);
        pVocab->SetCustomObjectReader(customShortArrayReader);
        pOuter->AddBuiltinClass("ShortArrayIter", kBCIShortArrayIter, kBCIIter, oShortArrayIterMembers);

        pVocab = pOuter->AddBuiltinClass("IntArray", kBCIIntArray, kBCIIterable, oIntArrayMembers);
        ...
        pOuter->AddBuiltinClass("StructArrayIter", kBCIStructArrayIter, kBCIIter, oStructArrayIterMembers);

        pOuter->AddBuiltinClass("Pair", kBCIPair, kBCIIterable, oPairMembers);
		pOuter->AddBuiltinClass("PairIter", kBCIPairIter, kBCIIter, oPairIterMembers);

		pOuter->AddBuiltinClass("Triple", kBCITriple, kBCIIterable, oTripleMembers);
		pOuter->AddBuiltinClass("TripleIter", kBCITripleIter, kBCIIter, oTripleIterMembers);
	}
